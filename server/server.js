...
const { Blob } = require("buffer"); // Required for creating a Blob
...
app.get("/profile/:id", async (req, res) => {
  try {
    ...
    if (!user) user = await Investor.findById(id).select("id username email type");
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    res.status(200).json({ success: true, user: { id: user._id, username: user.username, email: user.email, type: user.type } });
  } catch (error) {
...
app.post("/filterposts", filterAndValidatePost, async (req, res) => {
  try {
    ...
    try {
      ...
      const file = new File([blob], "fullIdea.json", { type: "application/json" });
      const upload = await pinata.upload.file(file); // Updated method
      console.log("Pinata upload response:", upload);
      ipfsHash = upload.cid; // Extract CID
      console.log("Pinata upload successful, CID:", ipfsHash);
    } catch (pinataError) {
      console.error("Pinata upload failed:", pinataError);
      console.warn("Using fallback CID for submission...");
